...
import { getAllInvoices, approveInvoice, rejectInvoice } from "@/services/topup.client";
...
import { FiEye, FiFilter, FiCheck, FiX, FiDollarSign, FiClock, FiUser, FiCreditCard, FiFileText } from "react-icons/fi";
...
const statusMap: Record<string, { label: string; color: string; dotColor: string }> = {
  ...
  CANCELLED: { label: "Dibatalkan", color: "bg-gray-50 text-gray-600 border-gray-200 dark:bg-gray-800/50 dark:text-gray-400 dark:border-gray-700", dotColor: "bg-gray-400" },
  REJECTED: { label: "Ditolak", color: "bg-red-50 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800", dotColor: "bg-red-500" },
};
...
export default function TopupsTable() {
  ...
  const filtered = invoices.filter((inv: TopupInvoice) => {
    const invDate = new Date(inv.createdAt);
    const matchDate =
      (!startDate || invDate >= new Date(startDate + 'T00:00:00')) &&
      (!endDate || invDate <= new Date(endDate + 'T23:59:59'));
    return (
      matchDate &&
  ...
  const detailInv = invoices.find((t: TopupInvoice) => t.id === detailId);
        ...
        <div className="flex flex-wrap gap-3 items-end bg-gray-50/50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-100 dark:border-gray-700">
          <div className="flex flex-col gap-1.5 w-full md:w-auto">
            ...
            <Input
              ...
              placeholder="Cari Kode Invoice / ID User..."
              className="w-full md:w-56 bg-white dark:bg-gray-900"
            />
          </div>
          ...
          <div className="flex flex-col gap-1.5 w-full md:w-auto mt-5">
            <Button
              ...
              onClick={() => { setStatus(""); setSearch(""); setStartDate(""); setEndDate(""); }}
            >
              <FiFilter /> Reset
            </Button>
          </div>
          ...
          <Table>
            <TableHeader className="bg-gray-50 dark:bg-gray-800/50">
              <TableRow>
                ...
                <TableHead className="whitespace-nowrap">ID User</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                paginated.map((inv: TopupInvoice, idx: number) => {
                  ...
                  return (
                    <TableRow key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <TableCell>
                        <span className="text-sm text-gray-700 dark:text-gray-300 max-w-[200px] truncate inline-block" title={inv.userId}>
                          {inv.userId.length > 20 ? `${inv.userId.slice(0, 8)}...${inv.userId.slice(-8)}` : inv.userId}
                        </span>
                      </TableCell>
                      <TableCell>
                        <span className="px-2 py-1 rounded bg-blue-50 text-blue-700 text-xs dark:bg-blue-900/30 dark:text-blue-300">
                          {inv.paymentMethod === 'BCA_MANUAL' ? 'BCA (Manual)' : inv.paymentMethod || '-'}
                        </span>
                      </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-12 text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm">Tidak ada top up yang sesuai dengan filter saat ini</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
      ...
      {!isLoading && paginated.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center px-6 pb-6 gap-4">
          <span className="text-sm text-gray-500">
            Menampilkan <span className="font-medium text-gray-900 dark:text-gray-100">{offset + 1}</span> hingga <span className="font-medium text-gray-900 dark:text-gray-100">{offset + paginated.length}</span> dari <span className="font-medium text-gray-900 dark:text-gray-100">{filtered.length}</span> invoice
          </span>
            ...
            <>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
                <div className="flex justify-between items-start">
                  <div>
                    <DialogTitle className="text-xl font-bold">Detail Top Up</DialogTitle>
              ...
              <div className="p-6 space-y-6 bg-white dark:bg-slate-900">
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-5 space-y-4 border border-gray-100 dark:border-gray-800">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <FiClock className="h-3 w-3" /> Tanggal
                      </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <FiUser className="h-3 w-3" /> ID User
                      </div>
                      <p className="font-semibold text-sm break-all text-gray-900 dark:text-gray-100">{detailInv.userId}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wider">
                      <FiCreditCard className="h-3 w-3" /> Metode Pembayaran
                    </div>
                    <p className="font-semibold text-sm text-gray-900 dark:text-gray-100">
                      {detailInv.paymentMethod === 'BCA_MANUAL' ? 'BCA (Manual)' : detailInv.paymentMethod || 'Transfer/Manual'}
                    </p>
                  </div>
                  ...
                  {detailInv.notes && (
                    <>
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wider">
                          <FiFileText className="h-3 w-3" /> Catatan
                        </div>
                        <div className="bg-white dark:bg-gray-900 p-2.5 rounded border border-gray-200 dark:border-gray-700 mt-1">
                          <p className="text-sm text-gray-900 dark:text-gray-100">{detailInv.notes}</p>
                        </div>
                      </div>
                    </>
                  )}
