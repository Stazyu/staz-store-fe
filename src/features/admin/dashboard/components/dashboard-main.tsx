...
const CustomAreaTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900/95 border border-slate-200 dark:border-white/10 rounded-xl p-3 shadow-2xl backdrop-blur-sm">
...
const CustomBarTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900/95 border border-slate-200 dark:border-white/10 rounded-xl p-3 shadow-2xl backdrop-blur-sm">
      <p className="text-sm text-slate-900 dark:text-white font-semibold">{payload[0].payload.name}</p>
      <p className="text-xs text-slate-500 dark:text-gray-400">{payload[0].value} transaksi</p>
    </div>
  );
};
...
function DashboardSkeleton() {
  return (
    <div className="space-y-8 pb-8 animate-pulse">
      <div className="relative overflow-hidden rounded-3xl p-8 bg-slate-200 dark:bg-gray-800/80 border border-slate-300 dark:border-gray-700 h-[280px]" />
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-32 bg-slate-100 dark:bg-gray-800/50 border border-slate-200 dark:border-gray-700 rounded-2xl" />
        ))}
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 h-[415px] bg-slate-100 dark:bg-gray-800/50 border border-slate-200 dark:border-gray-700 rounded-2xl" />
        <div className="h-[415px] bg-slate-100 dark:bg-gray-800/50 border border-slate-200 dark:border-gray-700 rounded-2xl" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      <div className="relative overflow-hidden rounded-3xl p-8">
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <LiveDot />
              <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Analitik Live</span>
            </div>
        ...
        {/* Metrics strip */}
        <div className="relative mt-8 grid grid-cols-3 gap-4 pt-6 border-t border-slate-200 dark:border-white/10">
          {[
            ...
            <div key={m.label} className="flex items-center gap-3">
              ...
              <div>
                <div className="text-slate-900 dark:text-white font-bold text-lg leading-none">
                  {typeof m.value === "number" ? m.value.toLocaleString("id-ID") : m.value}
                </div>
                <div className="text-slate-500 dark:text-blue-200/50 text-xs mt-0.5">{m.label}</div>
              </div>
            </div>
          ))}
        </div>
