    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-blue-500/20 dark:border-blue-500/10 bg-gradient-to-br from-blue-50 via-indigo-50 to-violet-50 dark:bg-none dark:bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,0.22),transparent_35%),linear-gradient(135deg,#07111f_0%,#081e3d_45%,#071a33_100%)] shadow-[0_20px_80px_rgba(37,99,235,0.08)] p-6">
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-900 dark:text-white leading-none">Deposit Saldo</h1>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Persetujuan manual dan riwayat pengisian saldo (top-up) customer.</p>
            </div>
          </div>
      ...
      <Card className="border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/40 backdrop-blur-md">
        <CardHeader className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            <CardTitle className="text-lg font-bold text-slate-900 dark:text-white">Daftar Deposit Saldo</CardTitle>
            <CardDescription className="text-xs text-slate-500 dark:text-slate-400">Tinjau dan proses permohonan topup saldo customer.</CardDescription>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <option value="PAID" className="bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-200">PAID (SUCCESS)</option>
              <option value="EXPIRED" className="bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-200">EXPIRED</option>
              <option value="CANCELLED" className="bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-200">CANCELLED</option>
            </select>
            <Input
              type="date"
              value={startDate}
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-slate-100 dark:border-slate-800 hover:bg-transparent">
                  ...
                  <TableHead className="text-slate-500 dark:text-slate-400 font-bold">Deposit ID / Invoice</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400 font-bold">Customer</TableHead>
                  <TableHead className="text-slate-500 dark:text-slate-400 font-bold">Metode Pembayaran</TableHead>
                  ...
                  <TableHead className="text-slate-500 dark:text-slate-400 font-bold">Total Bayar</TableHead>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={11} className="py-20 text-center text-slate-500">
                      <div className="flex flex-col items-center gap-2">
                        <Wallet className="size-10 text-slate-400 dark:text-slate-600" />
                        <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Belum ada data deposit</p>
                        <p className="text-xs text-slate-400 dark:text-slate-500">Tidak ada pengajuan deposit saldo yang cocok dengan filter saat ini.</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                  deposits.map((deposit, idx) => {
                    ...
                    return (
                      <TableRow key={deposit.id} className="border-slate-100 dark:border-slate-900 hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                        <TableCell className="text-center text-xs text-slate-500 dark:text-slate-400">{offset + idx + 1}</TableCell>
                        <TableCell className="font-mono text-xs font-semibold text-blue-600 dark:text-blue-400">{deposit.invoiceCode}</TableCell>
                        <TableCell className="whitespace-nowrap">
                          <p className="text-sm font-semibold text-slate-900 dark:text-slate-200">{deposit.user?.name || "Guest"}</p>
                          <p className="text-[10px] text-slate-400 dark:text-slate-500">{deposit.user?.email || ""}</p>
                        </TableCell>
                        <TableCell className="text-sm text-slate-600 dark:text-slate-300 font-medium">{deposit.paymentMethod}</TableCell>
                        <TableCell className="text-sm text-slate-600 dark:text-slate-300 font-medium">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${
                            deposit.topUpChannel === "TELEGRAM" 
                              ? "bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20" 
                              : "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20"
                          }`}>
                            {deposit.topUpChannel || "WEB"}
                          </span>
                        </TableCell>
                        <TableCell className="text-sm font-bold text-slate-900 dark:text-slate-200">
                          Rp {deposit.amount.toLocaleString("id-ID")}
                        </TableCell>
                        <TableCell className="text-sm text-slate-500 dark:text-slate-400">Rp 0</TableCell>
                        <TableCell className="text-sm font-bold text-slate-900 dark:text-slate-200">
                          Rp {deposit.amount.toLocaleString("id-ID")}
                        </TableCell>
                            ...
                            {deposit.status === "PENDING" && (
                              <>
                                <Button
                                  ...
                                  className="size-8 rounded-lg text-emerald-600 dark:text-emerald-500 hover:bg-emerald-500/10 hover:text-emerald-500 dark:hover:text-emerald-400"
                                  title="Approve"
                                >
                                ...
                                <Button
                                  ...
                                  className="size-8 rounded-lg text-rose-600 dark:text-rose-500 hover:bg-rose-500/10 hover:text-rose-500 dark:hover:text-rose-400"
                                  title="Reject"
                                >
          ...
          {activeDetailDeposit && (
            <div className="p-6 space-y-4 text-sm">
              <div className="flex justify-between items-center bg-slate-50/50 dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">{activeDetailDeposit.invoiceCode}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">ID: {activeDetailDeposit.id}</p>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  ...
                  <span className="text-slate-800 dark:text-slate-200">{activeDetailDeposit.user?.name || "Guest"}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  <span className="text-slate-500 dark:text-slate-400">Email Customer</span>
                  <span className="text-slate-800 dark:text-slate-200">{activeDetailDeposit.user?.email || "-"}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  <span className="text-slate-500 dark:text-slate-400">Metode</span>
                  <span className="text-slate-800 dark:text-slate-200">{activeDetailDeposit.paymentMethod}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  <span className="text-slate-500 dark:text-slate-400">Channel</span>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold ${
                    activeDetailDeposit.topUpChannel === "TELEGRAM"
                      ? "bg-sky-500/10 text-sky-600 dark:text-sky-400 border border-sky-500/20"
                      : "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20"
                  }`}>
                    {activeDetailDeposit.topUpChannel || "WEB"}
                  </span>
                </div>
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  <span className="text-slate-500 dark:text-slate-400">Reference ID</span>
                  <span className="font-mono text-slate-800 dark:text-slate-200">{activeDetailDeposit.paymentRef || "-"}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  <span className="text-slate-500 dark:text-slate-400">Tanggal Diajukan</span>
                  <span className="text-slate-800 dark:text-slate-200">{new Date(activeDetailDeposit.createdAt).toLocaleString("id-ID")}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 dark:border-slate-900 pb-2">
                  <span className="text-slate-500 dark:text-slate-400">Batas Waktu</span>
                  <span className="text-amber-600 dark:text-amber-500">{new Date(activeDetailDeposit.expiredAt).toLocaleString("id-ID")}</span>
                </div>
                <div className="flex justify-between pt-2">
                  <span className="text-slate-500 dark:text-slate-400 font-semibold">Biaya Layanan</span>
                  <span className="text-slate-800 dark:text-slate-200">Rp 0</span>
                </div>
              ...
              <div className="pt-4 flex gap-2">
                <Button variant="outline" className="flex-1 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-300" onClick={() => setDetailId(null)}>
                  Tutup
                </Button>
                {activeDetailDeposit.status === "PENDING" && (
                  <>
                    <Button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-550 dark:hover:bg-emerald-500 text-white font-semibold"
                    >
                      Setujui
                    </Button>
                  </>
        ...
        <DialogContent className="sm:max-w-md bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-900 dark:text-slate-200">
          ...
          <div className="py-4">
            <p className="text-sm text-slate-600 dark:text-slate-300">
              ...
              <span className="font-mono text-blue-600 dark:text-blue-400 font-bold">{actioningId}</span>?
            </p>
            {actionType === "approve" && (
              <p className="text-xs text-amber-600 dark:text-amber-505 mt-2 font-medium">
                ⚠️ Tindakan ini akan langsung menambahkan saldo ke akun customer.
              </p>
            )}
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => { setActioningId(null); setActionType(null); }} className="border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-700 dark:text-slate-300">
              Batal
            </Button>
            <Button
              onClick={confirmAction}
              className={actionType === "approve" ? "bg-emerald-600 hover:bg-emerald-550 text-white" : "bg-rose-600 hover:bg-rose-550 text-white"}
            >
